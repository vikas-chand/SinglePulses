...
\node[agent, below=of c3] (c4) {fresh-context sub-agents (isolate): the roster's verifiers and readers};
...
% arrows: loop
\draw[arr] ($(model.south)+(-8mm,0)$) -- ++(0,-1.5mm) -| (loop.north);
\draw[arr] (loop.east) -- (tools.west);
\draw[arr] (tools.east) -- (ctx.west);
\draw[arr] (ctx.east) -- (state.west);
\draw[arr] (state.east) -- (bound.west);
\draw[arr] (bound.north) |- ($(model.south)+(8mm,-1.5mm)$) -- ($(model.south)+(8mm,0)$);
\draw[arr, humanline] (pi.north) -- ++(0,1.5mm) -| ($(s4.east)+(1.5mm,0)$) |- (s2.east);
\node[lab, below=1mm of pi] (pilab) {the approval is identity-stamped into the records above; never fabricated};
...
\begin{scope}[on background layer]
  ...
  \node[fill=white, draw=agentline, rounded corners=1pt, inner sep=1.5pt, font=\sffamily\scriptsize\bfseries, text=agentline!80!black,
        text width=23mm, align=left, anchor=south west] at ($(repo.south west)+(1mm,0.6mm)$) {the repository layer: what makes the session a GRB analyst};
\end{scope}
