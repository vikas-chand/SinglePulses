...
%   inner green = the repository layer). Same grammar in F2/F4/F5.
...
\begin{tikzpicture}[
  ...
  code/.style={box, fill=codefill, draw=codeline},
  agent/.style={box, fill=agentfill, draw=agentline},
...
\node[lab, above=1mm of model, font=\sffamily\scriptsize] (toplab) {\textbf{agent = model + harness}; the harness is everything below};
...
\node[agent, below=of loop] (l1) {burst-walkthrough protocol: RUN $\to$ PRESENT $\to$ GATE $\to$ LITERATURE $\to$ DISTILL};
...
\node[code, below=of t2] (t3) {Stage-1 approval tool: detectors, background, source --- stamped};
\node[code, below=of t3] (t4) {figures, temporal chain, SED family, per-burst report};

\node[doc, below=of ctx] (c1) {27 guide files; 10 step skills indexed by the ledger; lessons with IDs};
...
\node[agent, below=of c3] (c4) {memory files + notes; fresh-context sub-agents (isolate)};
...
% the human gate
\node[human, below=6.5mm of s4.south, anchor=north] (pi) {PI gate: every step};
...
\draw[arr, humanline] (pi.north) -- ++(0,2mm) -| ($(s4.east)+(1.5mm,0)$) |- (s2.east);
\node[lab, anchor=south west, text=humanline] at ($(pi.north)+(3mm,2.4mm)$) {stamps};
...
\begin{scope}[on background layer]
  ...
  \node[draw=agentline, thick, rounded corners=3pt, pattern=north east lines, pattern color=agentline!25, inner sep=2.5mm,
        fit=(headroom)(loop)(bound)(l4)(b4)(t1)] (repo) {};
  \node[fill=white, draw=agentline, rounded corners=1pt, inner sep=1.5pt, font=\sffamily\scriptsize\bfseries, text=agentline!80!black,
        anchor=north] at ($(repo.north)+(0,-0.6mm)$) {the repository layer: what makes the session a GRB analyst};
  \node[draw=black!45, thick, rounded corners=4pt, pattern=north west lines, pattern color=black!12, inner sep=3mm,
        fit=(toplab)(model)(repo)(pi)(pilab)] (host) {};
...
\node[lab, right=2mm of leg5, text width=34mm, align=left, execute at begin node=\nohyph] {census 2026-09-02; each box maps to rows of Table~T1; hatched frames = the two harness layers};
\end{tikzpicture}
\end{document}
