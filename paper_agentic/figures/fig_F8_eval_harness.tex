...
\node[code, below=2.5mm of hr] (tests) {test suite: 134 tests (lessons-as-tests, id guards, compile checks) over the real products};
...
% the coupling
\draw[arr] (stamp.east) -- ++(4mm,0) |- node[lab, pos=0.25, right, text width=11mm, align=left] {any change to the runtime, or of the model} (tests.west);
\draw[arr, dashed] (rec.west) -- ++(-5mm,0) |- (next.east);
...
% legend
\node[code, below=9mm of next.south west, anchor=north west, text width=12mm, font=\sffamily\scriptsize] (l1) {blue: code};
