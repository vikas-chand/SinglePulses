...
that fixes generators by commit and products by hash. Ten object types
the pipeline writes carry a JSON schema\prov{}; five of the thirteen drawn
here are among them (the stamp, the receipt, the audit, the action event,
and the burst state), and the others are the literature-harvest manifest
and its paper records, the frozen prediction record of
\S\ref{sec:doctrine}, and the fit sidecar. The test suite validates every
...
writes a stamp; \emph{promote} and \emph{quarantine} write a receipt; and
\emph{invalidate} demotes the approvals downstream of an amended step and
records why. Two are proposed and drawn dashed. \emph{Launch producer}
