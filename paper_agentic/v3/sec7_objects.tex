...
products. Bottom row: the law that governs them. Colour says who writes
the object: blue by code (the audit box also holds the agent-written
vision and numbers verdicts), amber carrying an approver identity (human
on 433 of the 436 selection rows), white the law and documents; the roles
band uses the same colours, green for the agent. A diamond marks an
object type with a JSON schema on disk. Links are solid where a declared field
