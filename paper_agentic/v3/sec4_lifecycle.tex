...
% v2's fig:fitstep TikZ KEPT UNCHANGED as F3 (outline: "v2 Fig. 2 unchanged"), the fan-out/verifier-gates
...
finds and fetches every refereed paper that analysed the burst, using four
query forms (the GRB name, the trigger name, the day fraction, the trigger
...
runs the literature comparison, and closes the burst into one of a set of
typed terminal states, of which \texttt{non-identifiable} is a
\emph{successful} outcome: the workflow is forbidden to keep adjusting an
...
its criteria, its checklist, and its numbered lessons; for the selection
...
promoted table and names its result with the model identity and commit,
...
evidence ratio $e^{\Delta\mathrm{AIC}/2}$ and graded. A margin of 6
corresponds to roughly 20:1 and is called strong; 10, roughly 150:1,
decisive; anything less is reported as what it is. Heads within two units
...
these data alone. Two further stamps ride on the fit row rather than in
prose: a bound-pinned parameter is flagged where it sits, and a thermal
component whose peak falls below the lowest usable detector energy is
marked as edge-constrained, so that a reader of the table sees the
condition without reading the report.
