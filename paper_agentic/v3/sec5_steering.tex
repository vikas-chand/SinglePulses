...
% Counts (census 2026-09-03, numbers-verifier round 1 applied): 43 heading-form numbered lessons in FOUR step skills
% (L 31 of ids to 34 — L6b is a sub-lesson, not counted; TM 5; D 5; G 2); LiteratureHarvest carries 11 list-form traps;
% 23 credited to the nine walked bursts (ledger, ids whitelisted + deduplicated, L26 -> TM3; #21 by heading burst tag,
% PROPOSED excluded); register 49 NR rows: 20 DEPLOYED, 22 PROPOSED, 3 PARTIAL, 4 closed/practice/fixed (NR-25's
% status cell holds a literal \| — parse on the keyword); 9 rows with a keep: clause.
...
our scale, and one property they share. It is data-efficient: four of the
five step skills hold forty-four heading-form numbered lessons\prov{} (the
literature-harvest skill carries eleven numbered traps in list form);
twenty-three of the forty-four are credited to the nine bursts walked so
far, by the campaign ledger for the first eight and by the lesson headings
for the ninth, and the rest to the reconciliation pilot that preceded the
walkthroughs, to campaign batches, to lessons the ledger rows omit, to
rulings by the human, and to one lesson still proposed and pending the
human's ruling. It is auditable: the learned policy is plain
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=0.55\textwidth]{figures/fig_F7_learning_curve.pdf}
\caption{The campaign learning curve\prov: lessons credited per walked
burst (bars), defects found at any layer (dark bars), and the cumulative
lesson count (line). The first eight bars follow the campaign ledger, with
lesson identifiers checked against the skill files and counted once; the
ninth is the walkthrough burst still in progress, credited with the lessons
whose entries name it and are not marked proposed; its defects are not yet
tallied, marked by a cross rather than by a bar: the first two bursts, which
have no dark bar either, are measured zeros. The order is walk order, not time, and it coincides with the
review-index order of the sample only for the first three bursts; the
ninth burst is review-index 21. The curve is visibly unconverged; its
flattening, not a preset burst count, terminates the training phase.}
\label{fig:curve}
\end{figure*}
...
twenty-two are proposed, three are partial (the sensor side built, the
producer side not), and four are closed, recorded as practice, or fixed in
place. The
...
\startdata
\multicolumn{3}{l}{\textit{summary: 49 rows; 20 deployed, 22 proposed, 3 partial, 4 closed, practice, or fixed in place; 9 with a keep condition}} \\
...
NR-24 & \rc{report-conformance gate: an assembled deliverable verified against the report contract, whose first rule binds report and figures to the fit table's hash} & proposed \\
...
NR-48 & \rc{derived columns recomputed on cell replacement: a promoted table's stored winner must equal the validity-gated argmin over all model columns} & proposed (sensor deployed; promotion-path recompute not written) \\
\enddata
\end{deluxetable*}
...
that fails on any duplicate register identifier entered the suite. A second
collision found the same day, two skill files carrying the same lesson
numbers, produced a sibling test that guards the lesson prefixes of every
skill file. The incident cost nothing
