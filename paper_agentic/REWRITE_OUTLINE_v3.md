...
| 2026-09-03 ~13:25 | abstract (decision 23) | "Anyways, just write something, and abstract can anyway can be written at the end when paper is done" | r5 (brief, A&A flow, 195 words) spliced into v3 as PROVISIONAL; the final abstract is written after §1/§11 with the frozen-run numbers; AbstractHelper (retrieval corpus + metrics) is the tool for that pass |
| 2026-09-03 ~13:55 | colour figures (decision 24) | "yes, that's fine, but I have a question, if the proposed ones we want to build and not built then why?" | F1 r5 (7fcb79d0), F2 r2b (fa128aaa), F4 r3 (7e3dbcfe), F5 r2 (80f0cd5e) APPROVED in colour; the question on PROPOSED components answered in the session (gated-by-PI-rule vs queued-by-program-order vs one plain omission: the report-conformance agent file) |
| 2026-09-03 ~14:00 | PROPOSED components (decision 25) | "ok no problem, you can keep things on hold and as we test on more and more GRBs, their roles will become clearler and maybe we need more, we don't want to over engineering but also want to have more than essential" | the five PROPOSED rows stay proposed; nothing built ahead of need; each burst walked decides what gets built next; the paper states them as specified-not-built with their reason class at the next roster pass |
| 2026-09-03 ~14:20 | figure style pass (decision 26) | "Apply now, all four (Recommended)" | Source Sans Pro, corners 4–7 pt, 1 pt black!55 round arrows, light neutral frames; texts and D24 colours unchanged; four style gates in flight; PI re-approval after |
