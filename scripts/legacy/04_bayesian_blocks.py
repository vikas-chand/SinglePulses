...
BKG_INTERVALS_PATH = os.path.join(RESULTS_DIR, 'background_intervals.ecsv')
...
def load_bkg_table():
    """Load per-(trigger, det) background intervals written by 00_select_backgrounds.py."""
    if not os.path.exists(BKG_INTERVALS_PATH):
        raise FileNotFoundError(
            f"Per-detector background intervals not found at {BKG_INTERVALS_PATH}. "
            f"Run scripts/00_select_backgrounds.py first."
        )
    return Table.read(BKG_INTERVALS_PATH, format='ascii.ecsv')


def lookup_bkg(bkg_table, trigger, det):
    """Return (neg_start, neg_stop, pos_start, pos_stop) for one (trigger, det)."""
    mask = (bkg_table['TRIGGER_NAME'] == trigger) & (bkg_table['DETECTOR'] == det)
    rows = bkg_table[mask]
    if len(rows) == 0:
        raise KeyError(f"No background interval row for ({trigger}, {det})")
    r = rows[0]
...
if __name__ == '__main__':
    ...
    # Load full sample for T90_START and NAI_DETECTORS
    sample = Table.read(os.path.join(RESULTS_DIR, 'grb_sample.ecsv'),
                        format='ascii.ecsv')

    # Per-detector background intervals from 00_select_backgrounds.py
    bkg_table = load_bkg_table()
    ...
    for i, row in enumerate(single_pulse):
        ...
        # Get T90_START and NaI list from main sample
        ...
        # Per-detector background intervals for the brightest NaI
        try:
            bkg_neg_start, bkg_neg_stop, bkg_pos_start, bkg_pos_stop = \
                lookup_bkg(bkg_table, trigger, det)
        except KeyError as exc:
            print(f"[SKIP] {trigger} ({det}): {exc}")
            n_failed += 1
            continue
        ...
        try:
            ...
            for other_det in additional_dets:
                # Each detector has its own per-detector background interval
                try:
                    o_neg_a, o_neg_b, o_pos_a, o_pos_b = \
                        lookup_bkg(bkg_table, trigger, other_det)
                except KeyError as exc:
                    print(f"\n    [WARN] {other_det}: {exc}", end='')
                    continue
                try:
