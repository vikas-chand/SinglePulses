...
if n>=5:
    ...
    ax.set_title(f"Lag(sign-corr) vs MVT — single-pulse GRBs  ($\\rho$={rho:+.2f})",fontsize=12)
    fig.tight_layout(); fig.savefig("temporal_properties/figures/lag_vs_mvt_signcorr.png",dpi=180,bbox_inches="tight")
    out.append("wrote temporal_properties/figures/lag_vs_mvt_signcorr.png")
else:
    out.append("too few points for the correlation (<5) — check the survey output / cleanness")
open("temporal_properties/results/lag_mvt_stats_signcorr.txt","w").write("\n".join(out)+"\n")
print("\n".join(out))
