# bn081125496 — reproduction record
...
| `47b_temporal_figs.py` | `33a38046c3f62abf…` | `e01907d225fa7a91…` | **NO — script changed since render** |
| `47c_lag_latbright.py` | `275d82c78088d9cc…` | `275d82c78088d9cc…` | YES |

## Exact commands (argv recorded per product)

```
python3 scripts/41e_sed_montage.py --trig bn081125496
python dev/rebuild_step9_canonical.py --trig bn081125496
python3 dev/gen_param_tables.py --trig bn081125496
python scripts/10_spectral_fit_burst.py --trigger bn081125496 --include-bgo --no-log --models highe \
    --blocks-file results/sweep106/bn081125496/blocks/bb_blocks_spectral_bn081125496.ecsv \
    --bkg-file results/background_intervals.ecsv --out-dir results/campaign20_fam/bn081125496_highe
```
...
|---|---|---|---|---|
| na | (-23.50, -8.00) | (30.00, 140.00) | (-1.70, 11.90) | Vikas Chand |
| nb | (-20.96, -5.96) | (17.04, 75.04) | (-1.70, 11.90) | Vikas Chand |
| b1 | (-20.96, -5.96) | (21.04, 81.04) | (-1.70, 11.90) | Vikas Chand |

Block table: `results/sweep106/bn081125496/blocks/bb_blocks_spectral_bn081125496.ecsv`
- sha256 `5c388552b4677589…`
- reused unchanged from the 2026-08-12 binning run; re-deriving blocks
  per convention would break cross-burst comparability.

Canonical fit table: `results/convention_check/bn081125496/spectral_fits.ecsv`
- sha256 `ef96e8eaaba03def…`; 24 models × 10 spectra
- canonicalization: best optimizer minimum per (bin, model) across
  all invocations we possess (multistart luck reaches ΔAIC ≈ 9).

## Notebook

`notebooks/Two_Breaks_single_GRB_pipeline.ipynb` (config `notebooks/configs/bn081125496.yaml`) runs the same chain interactively and
