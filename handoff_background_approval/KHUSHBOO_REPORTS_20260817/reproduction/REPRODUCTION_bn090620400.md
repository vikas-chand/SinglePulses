# bn090620400 — reproduction record
...
```
python scripts/41c_paper_sed.py --trig bn090620400 --bin 0 --model BAND --out results/convention_check/sed_grid_bn090620400 --fit-root results/convention_check
python scripts/41c_paper_sed.py --trig bn090620400 --bin 0 --model BANDBB --out results/convention_check/sed_grid_bn090620400 --fit-root results/convention_check
python scripts/41c_paper_sed.py --trig bn090620400 --bin 0 --model BANDBBCPL --out results/convention_check/sed_grid_bn090620400 --fit-root results/convention_check
python scripts/41c_paper_sed.py --trig bn090620400 --bin 0 --model BANDBBPL --out results/convention_check/sed_grid_bn090620400 --fit-root results/convention_check
python scripts/41c_paper_sed.py --trig bn090620400 --bin 0 --model BANDCPL --out results/convention_check/sed_grid_bn090620400 --fit-root results/convention_check
python scripts/41c_paper_sed.py --trig bn090620400 --bin 0 --model BANDCUT --out results/convention_check/sed_grid_bn090620400 --fit-root results/convention_check
# … 234 more (one per model/bin panel; same form)
python3 scripts/41e_sed_montage.py --trig bn090620400
python dev/rebuild_step9_canonical.py --trig bn090620400
python3 dev/gen_param_tables.py --trig bn090620400
python scripts/10_spectral_fit_burst.py --trigger bn090620400 --include-bgo --no-log --models highe \
    --blocks-file results/sweep106/bn090620400/blocks/bb_blocks_spectral_bn090620400.ecsv \
    --bkg-file results/background_intervals.ecsv --out-dir results/campaign20_fam/bn090620400_highe
```
...
|---|---|---|---|---|
| n6 | (-32.65, -17.94) | (52.70, 132.70) | (-0.91, 18.27) | Vikas Chand |
| n7 | (-27.66, -5.66) | (34.34, 124.34) | (-0.91, 18.27) | Vikas Chand |
| n8 | (-32.65, -17.94) | (52.70, 132.70) | (-0.91, 18.27) | Vikas Chand |
| nb | (-26.65, -8.65) | (28.35, 93.35) | (-0.91, 18.27) | Vikas Chand |
| b1 | (-23.66, -6.66) | (26.34, 101.34) | (-0.91, 18.27) | Vikas Chand |

Block table: `results/sweep106/bn090620400/blocks/bb_blocks_spectral_bn090620400.ecsv`
- sha256 `7439de86f3044b89…`
- reused unchanged from the 2026-08-12 binning run; re-deriving blocks
  per convention would break cross-burst comparability.

Canonical fit table: `results/convention_check/bn090620400/spectral_fits.ecsv`
- sha256 `a18501a9684bd152…`; 24 models × 10 spectra
- canonicalization: best optimizer minimum per (bin, model) across
  all invocations we possess (multistart luck reaches ΔAIC ≈ 9).

## Notebook

`notebooks/Two_Breaks_single_GRB_pipeline.ipynb` (config `notebooks/configs/bn090620400.yaml`) runs the same chain interactively and
