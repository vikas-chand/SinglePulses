...
- **Source on pure background (CRITICAL).** In the LC PNG, the gap between the gold spans must straddle the obvious burst peak. Flag `source_on_background` if the marked source window contains **no peak above the background baseline** — i.e. the source-region rate ≈ the pre/post baseline rate (ratio ≲ 1.3). This is the exact bug that made 6 bursts' fits meaningless (bn090620400, bn090719063, bn100612726, bn100614498, bn110920546, bn200524211 in project memory) and collapsed them to 1 Bayesian block. A burst with **only 1 fitted block** is a strong corroborating signal. **[RESOLVED annotation, 2026-08-30 — CONFLICT-5 / NR-27 instance, history kept:** that six-burst list is the June-2026 background-misplacement era (memory `project_background_misplacement_fix`), fixed by `background_intervals_clean.ecsv`; it is NOT a live flag list. bn110920546's stamped Stage-1 (`results/approval/bn110920546_decision.json`) yields 11 blocks at S = 5.4–67.8, so `source_on_background` does not apply to it today. A skill-reader must read this list as a dated example of the failure class, never as current state; the current state of any burst is its stamped decision.json + `human_review_qc_flags.txt`.]
